...
// There are two decks: the main deck and the discard deck
// A Tower is an array of 10 spaces where bricks can be placed from the deck
type Deck = Brick[]
type Tower = Space[]

// A brick can be in one of four statuses: in the computer's tower, in the player's tower, in the main deck, or in the discard deck
type BrickStatus = "computer" | "player" | "main" | "discard"
...
class Brick implements BrickInterface {
  ...
  constructor(value: number, status: BrickStatus) {
    this.value = value
    this.status = status
  }
...
// There are two towers: the computer's tower and the player's tower
type TowerStatus = "computer" | "player"
...
class Space implements SpaceInterface {
  ...
  constructor(index: number, tower: TowerStatus, brick: Brick) {
    this.index = index
    this.tower = tower
    this.brick = brick
  }
...
class Game implements GameInterface {
  ...
  setupTowers(mainDeck: Deck) {
    const computerTower: Tower = [];
    for (let i = 0; i < 10; i++) {
  ...
  findAndReplaceBrick(index: number, tower: Tower, currentBrick: Brick, currentDeck: Deck, discardDeck: Deck) {
  ...
  findFit(tower: Tower, brick: Brick, towerSegment: string, currentDeck: Deck) {
    if (towerSegment == "top") {
  ...
  computerGameLoop(playerTower: Tower, computerTower: Tower, mainDeck: Deck, discardDeck: Deck) {
    // Computer's turn:
  ...
  gameLoop(playerTower: Tower, computerTower: Tower, mainDeck: Deck, discardDeck: Deck) {
    // Game Loop: 
